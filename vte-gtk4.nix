# TODO: Remove this in favour of using `nixpkgs`' `vte`.
# The following is copied from `nixpkgs` in an attempt to avoid needing to use
# a forked nixpkgs as a flake input.
{ stdenv
, lib
, fetchFromGitLab
, fetchurl
...
stdenv.mkDerivation rec {
  ...
  src = if vte-src != null then vte-src else fetchFromGitLab {
